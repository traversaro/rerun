...
use re_log_types::{
    component_types::{ ClassId, Tensor, TensorDataMeaning, TensorData },
    DecodedTensor,
    TensorElement,
};

use crate::{ misc::{ caches::TensorStats, ViewerContext }, ui::annotations::AnnotationMap };
...
impl EntityDataUi for Tensor {
    fn entity_data_ui(
        ...
        match ctx.cache.decode.try_decode_tensor_if_necessary(self.clone()) {
            Ok(decoded) => {
...
fn tensor_ui(
    ...
    let texture_result = crate::gpu_bridge
        ...
        UiVerbosity::All | UiVerbosity::Reduced => {
            ui.vertical(|ui| {
                ...
                if let Some(texture) = &texture_result {
                    ...
                    if
                        _encoded_tensor.data.is_compressed_image() ||
                    ...
                    if let Some([_h, _w, channels]) = tensor.image_height_width_channels() {
                        if channels == 3 {
                            if
                                let re_log_types::component_types::TensorData::U8(data) =
                                    &tensor.data
                            {
...
fn show_image_at_max_size(
    ...
    if
        let Err(err) = crate::gpu_bridge::render_image(
            render_ctx,
            &painter,
            response.rect,
            colormapped_texture,
...
pub fn tensor_summary_ui_grid_contents(
    ...
    re_ui.grid_left_hand_label(ui, "Shape").on_hover_text("Extent of every dimension.");
    ui.vertical(|ui| {
    ...
    if let Some((min, max)) = range {
        ui.label("Data range").on_hover_text("All values of the tensor range within these bounds.");
        ui.monospace(
            format!("[{} - {}]", re_format::format_f64(*min), re_format::format_f64(*max))
        );
        ui.end_row();
    }
}
    ...
    response.on_hover_cursor(egui::CursorIcon::Crosshair).on_hover_ui_at_pointer(|ui| {
        ...
        ui.horizontal(|ui| {
            if let Some([h, w, _]) = tensor.image_height_width_channels() {
                ...
                show_zoomed_image_region_area_outline(parent_ui, tensor, center_texel, image_rect);
...
pub fn show_zoomed_image_region(
    ...
    if
        let Err(err) = try_show_zoomed_image_region(
            render_ctx,
            ui,
            tensor,
            tensor_stats,
...
fn try_show_zoomed_image_region(
    ...
    {
        let center_texel_rect = egui::Rect::from_center_size(
            zoom_rect.center(),
            Vec2::splat(POINTS_PER_TEXEL)
        );
...
fn tensor_pixel_value_ui(
    ...
    egui::Grid::new("hovered pixel properties").show(ui, |ui| {
        ...
        if tensor.num_dim() == 2 {
            if let Some(raw_value) = tensor.get(&[y, x]) {
                if
                    let (TensorDataMeaning::ClassId, Some(u16_val)) = (
                        tensor.meaning(),
    ...
    let text = match tensor.num_dim() {
        2 =>
            match &tensor.data {
                TensorData::NV12(_) =>
                    tensor.get_nv12_pixel(&[y, x]).map(|[r, g, b]| {
                        match (r, g, b) {
...
fn save_image(tensor: &re_log_types::component_types::Tensor, dynamic_image: &image::DynamicImage) {
    ...
    match &tensor.data {
        TensorData::JPEG(bytes) => {
            if let Some(path) = rfd::FileDialog::new().set_file_name("image.jpg").save_file() {
        ...
        _ => {
            if let Some(path) = rfd::FileDialog::new().set_file_name("image.png").save_file() {
